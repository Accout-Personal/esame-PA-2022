"use strict";
exports.__esModule = true;
exports.generatePDF = void 0;
var PDFDocument = require("pdfkit");
function generatePDF() {
    var doc = new PDFDocument({ pdfVersion: '1.7' });
    // Metadati utilizzati per ottenere un documento di tipo PDF/A
    var SRGB_IEC61966_ICC_PROFILE_B64 = '';
    var xmp = "\n<?xpacket begin=\"\uFEFF\" id=\"W5M0MpCehiHzreSzNTczkc9d\"?>\n<x:xmpmeta xmlns:x=\"adobe:ns:meta/\">\n  <rdf:RDF xmlns:rdf=\"http://www.w3.org/1999/02/22-rdf-syntax-ns#\">\n      <rdf:Description xmlns:pdfaid=\"http://www.aiim.org/pdfa/ns/id/\" rdf:about=\"\">\n          <pdfaid:part>3</pdfaid:part>\n          <pdfaid:conformance>B</pdfaid:conformance>\n      </rdf:Description>\n  </rdf:RDF>\n</x:xmpmeta>\n<?xpacket end=\"w\"?>\n";
    // PDF/A standard richiede schema colore integrato.
    var colorProfile = Buffer.from(SRGB_IEC61966_ICC_PROFILE_B64, 'base64');
    var refColorProfile = doc.ref({
        Length: colorProfile.length,
        N: 3
    });
    refColorProfile.write(colorProfile);
    refColorProfile.end();
    var refOutputIntent = doc.ref({
        Type: 'OutputIntent',
        S: 'GTS_PDFA1',
        Info: new String('sRGB IEC61966-2.1'),
        OutputConditionIdentifier: new String('sRGB IEC61966-2.1'),
        DestOutputProfile: refColorProfile
    });
    refOutputIntent.end();
    // Metadata definisce tipo di documento.
    var metadata = xmp.trim();
    var refMetadata = doc.ref({
        Length: metadata.length,
        Type: 'Metadata',
        Subtype: 'XML'
    });
    refMetadata.compress = false;
    refMetadata.write(Buffer.from(metadata, 'utf-8'));
    refMetadata.end();
    // aggiunge manualmente oggetti creati nel catalogo
    doc._root.data.OutputIntents = [refOutputIntent];
    doc._root.data.Metadata = refMetadata;
    // PDF/A standard richiede font integrato
    //resources\fonts\OpenSans-Regular.ttf
    doc.registerFont('OpenSans', "./resources/fonts/OpenSans-Regular.ttf");
    return doc;
}
exports.generatePDF = generatePDF;
